  ...
  handleSearch(value) {
    if (this.state.object) {
      ...
      if (filtered.length > 0) {
        this.setState((prevState, props) => ({content: filtered}))
      }
    } else {
      let filtered = []
      ...
      if (this.state.object) {
        menu = this.state.content.map((item, index) => {
          if (item[this.state.name].length > 0) {
            return <a key={index} className={this.state.select === index
                ? 'dropdown-item is-active'
                : 'dropdown-item'} onClick={this.handleSelect.bind(this, index)}>{item[this.state.name]}</a>
          } else {
            return null
          }
        })
        ...
        menu = this.state.content.map((item, index) => {
          if (item.length > 0) {
            return <a key={index} className={this.state.select === index
                ? 'dropdown-item is-active'
                : 'dropdown-item'} onClick={this.handleSelect.bind(this, index)}>{item}</a>
          } else {
            return null
          }
        })
